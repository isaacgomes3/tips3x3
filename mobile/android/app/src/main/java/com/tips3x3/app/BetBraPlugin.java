...
public class BetBraPlugin extends Plugin {
  ...
  private static final String PREFS = BuildConfig.BOLSA_ONLY ? BolsaApostaExchangeClient.PREFS : "tips3x3_betbra";
  ...
  public void openLogin(PluginCall call) {
    ...
    String url = BuildConfig.BOLSA_ONLY
        ? "https://bolsadeaposta.bet.br/b/exchange"
        : call.getString("url", EXCHANGE_URL);
    if (!BuildConfig.BOLSA_ONLY
        && (url == null || url.isEmpty() || url.equals(SITE) || url.equals(SITE + "/"))) {
      url = EXCHANGE_URL;
    }
    Intent intent = new Intent(getContext(), BetBraLoginActivity.class);
    intent.putExtra(BetBraLoginActivity.EXTRA_URL, url);
    if (BuildConfig.BOLSA_ONLY) intent.putExtra(BetBraLoginActivity.EXTRA_VENUE, "bolsa");
  ...
  private void savePersistedToken(String token) {
    ...
    AutoLayForegroundService.prefs(getContext())
        .edit()
        .putBoolean(AutoLayForegroundService.PREF_SESSION_BLOCKED, false)
        .putBoolean("betbra_session_drop_notified", false)
        .apply();
  }
