        ...
        () -> {
          try {
            ...
            for (int i = 0; i < arr.length(); i++) {
              ...
              // Abertas / parciais / casadas ainda “vivas” no book
              boolean countable =
                  status.isEmpty()
              ...
              if (summary.length() < 400) {
                String event =
                    o.optString(
                        ...
                        o.optString(
                            "eventName",
                            o.optString("event_name", "")));
                double stake =
                    o.optDouble(
                        "stake",
                        o.optDouble("size", o.optDouble("size-remaining", 0)));
                double odds =
                    o.optDouble(
                        "odds",
                        o.optDouble("price", o.optDouble("odds-requested", 0)));
                if (summary.length() > 0) summary.append(" · ");
                summary
                    .append(event.isEmpty() ? ("#" + open) : event)
                    .append(" ")
                    .append(String.format(Locale.US, "%.2f@%.2f", stake, odds));
              }
            }
            JSObject out = new JSObject();
            out.put("ok", true);
            ...
            String raw = res.body;
            out.put("raw", raw != null && raw.length() < 8000 ? raw : "[]");
            resolveOnMain(call, out);
          } catch (Exception e) {
    ...
    io.execute(
        () -> {
          try {
            ...
            if (balance == null) balance = loadPersistedBalance(BALANCE_CACHE_MS);
            ...
            if (openExposure > 0.01 && available >= openExposure) {
              available = Math.round((available - openExposure) * 100.0) / 100.0;
            }
              ...
              if (spendable < 1) {
                rejectOnMain(
                    ...
                    openExposure > 0.01
                        ? String.format(
                            Locale.US,
                            "Saldo livre insuficiente (R$ %.2f livre · R$ %.2f em ofertas). Cancele uma oferta ou deposite.",
                            spendable,
                            openExposure)
                        : "Saldo indisponível ou insuficiente — toque em Atualizar na Bolsa Exchange");
                return;
              }
            ...
            out.put("openExposure", openExposure);
