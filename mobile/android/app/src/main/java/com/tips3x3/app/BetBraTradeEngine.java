...
final class BetBraTradeEngine {
  ...
  JSONObject getLayMatchDetails(String eventId, String marketId, String runnerId) {
    return getOfferMatchDetails("lay", eventId, marketId, runnerId);
  }

  /**
   * Confirma uma oferta Back ou Lay pela seleção exata e devolve apenas os
   * valores efetivamente correspondidos. Uma ordem que sumiu do book nunca é
   * tratada como casada: ela pode ter sido cancelada ou expirada.
   */
  JSONObject getOfferMatchDetails(
      String expectedSide, String eventId, String marketId, String runnerId) {
    try {
      ...
      String wantedSide = "back".equalsIgnoreCase(expectedSide) ? "back" : "lay";
      ...
      for (int i = 0; i < arr.length(); i++) {
        ...
        if (o == null) continue;
        String side = o.optString("side", o.optString("type", "")).toLowerCase(Locale.ROOT);
        if (!side.isEmpty() && !side.contains(wantedSide)) continue;
        String eid =
            o.optString(
                "event-id",
        ...
        if ("open".equals(kind) || "partial".equals(kind) || "unknown".equals(kind)) {
          ...
          JSONObject detail = extractMatchedOffer(o, wantedSide);
          if (detail != null) {
            bestMatched = detail;
          }
        }
  ...
  JSONObject getBackMatchDetails(String eventId, String marketId, String runnerId) {
    return getOfferMatchDetails("back", eventId, marketId, runnerId);
  }
  ...
  private static JSONObject extractMatchedOffer(JSONObject o, String side) {
    try {
      ...
      if ("lay".equals(side) && liability < 0.01 && stake > 0 && odds > 1.01) {
