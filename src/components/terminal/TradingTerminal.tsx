...
import { ExchangeOpsDesk } from "@/components/terminal/ExchangeOpsDesk";
...
export function TradingTerminal({
  ...
  /** APK: home operacional limpa (sem alocação/meta/timeline/extrato tipado). */
  if (exchange != null) {
    return (
      <div className="term-root is-ops">
        <ExchangeOpsDesk
          ...
          activeTrade={exchange.activeTrade ?? null}
          lastMessage={
            exchange.lastLay
              ? `${exchange.lastLay.ok ? "ok" : "falhou"}${
                  exchange.lastLay.score ? ` · ${exchange.lastLay.score}` : ""
                }${exchange.lastLay.message ? ` · ${exchange.lastLay.message}` : ""}`
              : null
          }
          onConnect={exchange.onConnect}
          onRefresh={exchange.onRefresh}
          onOpenSettings={exchange.onOpenSettings}
        />
        {inplay.length > 0 ? (
          <section className="term-dynamic-stack" style={{ marginTop: "0.85rem" }}>
            <article className="term-panel term-live-panel">
              <header className="term-panel-head">
                <h3>Ao vivo monitorados</h3>
                <span className="term-live-count">{inplay.length}</span>
              </header>
        ...
        <footer className="term-foot">
          <span>
            <i className="term-dot is-live" /> Operacional
          </span>
          <span>Última sync · {lastSyncSec}s</span>
        </footer>
      </div>
    );
  }
