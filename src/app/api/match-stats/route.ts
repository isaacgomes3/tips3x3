...
export async function GET(request: Request) {
  try {
    ...
    const fotmobMinute =
      parseLiveMinute(fotmob?.status) ??
      parseLiveMinute(fotmob?.rich?.status ?? null);
    const betbraMinute =
      parseLiveMinute(inplay?.elapsedRegularTime) ??
      parseLiveMinute(inplay?.timeElapsed);
    ...
    const body = {
      ...
      minute: pickTrustedLiveMinute({ betbraMinute, fotmobMinute }),
