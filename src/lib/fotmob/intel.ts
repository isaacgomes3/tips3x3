...
const intelCache = new Map<string, { at: number; value: MatchIntel | null }>();
const INTEL_TTL_MS = 12_000;
...
export async function getFotmobMatchIntel(opts: {
  ...
  const cacheKey = `${opts.home}|${opts.away}|${day}`.toLowerCase();
  const cached = intelCache.get(cacheKey);
  if (cached && Date.now() - cached.at < INTEL_TTL_MS) {
    return cached.value;
  }

  const value = await fetchFotmobMatchIntel(opts, day);
  intelCache.set(cacheKey, { at: Date.now(), value });
  return value;
}

async function fetchFotmobMatchIntel(
  opts: { home: string; away: string; start?: string },
  day: string,
): Promise<MatchIntel | null> {
