...
export async function scanDayOpportunities(options?: {
  ...
  for (let i = 0; i < listed.events.length; i += chunkSize) {
    ...
    for (const event of detailed) {
      ...
      opportunities.push({
        analysis,
        mexchangeUrl: analysis.marketId
          ? mexchangeEventUrl(event.id, analysis.marketId)
          : "",
        ...
        overMexchangeUrl35: analysis.overLimite35.marketId
          ...
          : undefined,
        overMexchangeUrl45: analysis.overLimite45.marketId
          ? mexchangeEventUrl(event.id, analysis.overLimite45.marketId)
          : undefined,
      });
    }
  }
