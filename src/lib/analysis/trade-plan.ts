...
export interface TradePlan {
  ...
  window: {
    ...
    max: number;
    preferredMax: number;
    /** Max base (sem super favorito). */
    baseMax: number;
    /** Max com super favorito. */
    superMax: number;
  };
  ...
  /** Favorito inicial + elegibilidade do placar live */
  scoreGate: {
    favoriteOdd: number | null;
    favoriteSide: "home" | "away" | null;
    favoriteOk: boolean;
    /** Informativo; entrada não usa superMax. */
    superFavorite: boolean;
  ...
  /** Gate xG: equilibrado (ex. 1.91–2.12) bloqueia indicação. */
  xgGate: XgBalanceGate;
  entryReady: boolean;
  summary: string;
}
...
export function buildTradePlan(opts: {
  ...
  teamForm?: TeamFormReport | null;
  /** xG live (FotMob/Sofa) — casa/fora. */
  xg?: XgSides | null;
}): TradePlan {
  const baseWindow = getLayOddsWindow();
  ...
  const superFavorite = favorite.isSuper === true;
  // Janela dura de ENTRADA: sempre min–max (padrão 20–50).
  // Super favorito NÃO abre faixa 50–100 (isso gerava Lay 90 indevido no Auto Lay).
  const window = {
    min: baseWindow.min,
    max: baseWindow.max,
    preferredMax: baseWindow.preferredMax,
    baseMax: baseWindow.max,
    superMax: baseWindow.superMax,
  };
  const inEntryWindow =
    layOdds != null &&
    ...
    layOdds >= window.min &&
    layOdds <= window.max;
  /** Só informativo — não libera entryReady. */
  const inExtendedBand =
    superFavorite &&
    layOdds != null &&
    layOdds > window.baseMax &&
    layOdds <= window.superMax;
  ...
  // Super favorito mitiga risco alto (inclui faixa estendida 50–100).
  const riskOk =
    risk.tier !== "alto" || crashEarlyBounce || formConfirms || superFavorite;

  const xgGate = evaluateXgBalanceGate(opts.xg, {
  ...
  const windowLabel = `${window.min}–${window.baseMax}`;

  let summary: string;
  if (layOdds == null) {
    summary = "Sem odd lay 3-3 de referência.";
  } else if (!liveScore) {
    summary = "Aguardando placar live — sem placar não libera.";
  } else if (!scoreGate.scoreAllowed) {
    summary = `Placar ${scoreGate.scoreLabel}: ${scoreGate.detail}`;
  } else if (!xgGate.allowed) {
    summary = xgGate.detail;
  } else if (crashSetup && crash) {
    if (!inEntryWindow && crash.phase === "trough") {
      summary = `${crash.detail} Odd atual ${layOdds.toFixed(0)} ainda fora de ${windowLabel} — se corrigir para baixo na janela ou subir após tick ↑, reavaliar.`;
    } else if (!inEntryWindow) {
      summary = `${crash.detail} Lay ${layOdds.toFixed(0)} fora da janela ${windowLabel}.`;
    ...
    summary = inExtendedBand
      ? `Lay ${layOdds.toFixed(0)} acima do máx. ${window.baseMax} (janela ${windowLabel}). Super favorito não amplia entrada.`
      : `Lay ${layOdds.toFixed(0)} fora da janela ${windowLabel}. Preferida ≤${window.preferredMax} (só informa).`;
  ...
  } else if (risk.tier === "alto" && (formConfirms || superFavorite)) {
    summary = entryReady
      ? `ENTRADA: lay ~${layOdds.toFixed(0)} (risco mitigado${superFavorite ? " pelo super favorito" : " pela forma"}). ${teamForm?.detail ?? ""}`
      : `Lay ${layOdds.toFixed(0)} alta, mas ${superFavorite ? "super favorito" : "forma"} mitiga. ${teamForm?.detail ?? ""} Aguardando fluidez/correção.`;
  ...
  return {
    ...
    window: {
      ...
      baseMax: window.baseMax,
      superMax: window.superMax,
    },
    targetBackOdds,
