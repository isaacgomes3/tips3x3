...
function buildExitPlan(opts: {
  ...
  minute?: number | null;
  favoritePressureBias?: number | null;
  /** Fração (0.005 = 0,5%). Meta do painel/admin — prioridade sobre o default. */
  ...
  const panelPct =
    ...
    opts.targetProfitPct > 0
      ...
      : null;
  // Meta do painel: usa o % pedido (com piso). Sem meta: dinâmica antiga.
  const targetProfitPct = panelPct
    ? clamp(panelPct, OVER_LIMITE.exit.minProfitPct, 0.05)
    : clamp(
...
export function buildOverLimiteSnapshot(opts: {
  ...
  minute?: number | null;
  /** Linha do Over (2.5, 3.5, 4.5…). Default: OVER_LIMITE.line */
  line?: number;
  /** Fração de lucro alvo (0.005 = 0,5%) para saída Back. */
  targetProfitPct?: number | null;
  matchOdds?: {
  ...
  const maxEntryGoals = OVER_LIMITE.maxEntryGoalsByLine[line];
  const scoreGateOk =
    maxEntryGoals == null
      ? true
  ...
  const entryReady =
    !settled && criticalOk && momentumOk && scoreGateOk;
  ...
  const summary = settled
    ...
    : !scoreGateOk
      ...
      : entryReady
        ? `Lay Over ${line} · ${scoreLabel ? `${scoreLabel} · ` : ""}${gateLabel} · ${goodCount}/${indicators.length} ok`
        : `Lay Over ${line} · ${scoreLabel ? `${scoreLabel} · ` : ""}${gateLabel} · ${goodCount}/${indicators.length} ok — caçar correção`;
