...
export interface LiveSnapshot {
  ...
  homeRedCards: number;
  awayRedCards: number;
  /** Total de vermelhos no jogo (soma dos lados ou agregado do feed). */
  redCards: number;
}
...
export interface LiveConfirmation {
  ...
  alerts: Array<{
    ...
    message: string;
    at: string;
    /** Dona do alerta — define a precedência de exibição (strategy-priority). */
...
function toCount(v: number | undefined | null) {
  const n = Number(v);
...
export function toLiveSnapshot(event: InplayEvent): LiveSnapshot {
  ...
  // Parser compartilhado (12', 45+2, HT) — Number("12'") virava NaN.
  const minute =
    minuteRaw != null && minuteRaw !== ""
      ? parseLiveMinute(minuteRaw) ?? toInt(minuteRaw, NaN)
      : null;
  ...
  const homeRedCards = toCount(event.score?.home?.numberOfRedCards);
  const awayRedCards = toCount(event.score?.away?.numberOfRedCards);
  const redCards = Math.max(
    homeRedCards + awayRedCards,
    toCount(event.score?.numberOfRedCards),
  );
