...
function redCardsLabel(n: number): string {
  return n > 1 ? `${n} cartões vermelhos` : "1 cartão vermelho";
}
 ...
 * - diff &lt; 3 → só últimos 5' antes do fim
 */
 ...
 * - stillPossible: 2T + (diff≥3 ou últimos 5') + time-gate B + bookOk (+ modelo)
 */
...
function buildLateWindowIndicator(
  ...
  if (EVENTOS_RAROS.blockOnRedCard && redCards > 0) {
    return {
      id: "late-window",
      label: m.label,
      icon: m.icon,
      tone: "bad",
      good: false,
      detail: `${redCardsLabel(redCards)} — padrão não alerta`,
      value: minute,
    };
  }
...
export function buildEventosRarosSnapshot(opts: {
  ...
  teamForm?: TeamFormReport | null;
  isLive?: boolean;
  /** Vermelhos no jogo. Bloqueia o padrão, não o LUCRO CERTO. */
  ...
  const redCards =
    opts.redCards != null && Number.isFinite(opts.redCards)
      ? Math.max(0, Math.trunc(opts.redCards))
      : 0;
  const redCardBlocked = EVENTOS_RAROS.blockOnRedCard && redCards > 0;
  ...
  if (isLive && (hs == null || as == null || minute == null)) {
    if (!blockers.some((b) => /incompleto/i.test(b))) {
  ...
  /** Timing do padrão (ainda possível): 2T + (diff≥3 ou últimos 5') + forma. */
  ...
  if (redCardBlocked) {
    blockers.push(
      `${redCardsLabel(redCards)} · padrão bloqueado (LUCRO CERTO segue)`,
    );
  }
  ...
  } else if (!indicationBlocked) {
    // Limpa blockers de late que não se aplicam
    blockers.length = 0;
  }

  // Hard blockers (amistoso / conflito) sempre visíveis.
  if (indicationBlocked) {
    for (const b of hardBlockers) {
      if (!blockers.includes(b)) blockers.unshift(b);
    }
  }
  ...
  const entryReady = !settled && !indicationBlocked && entries.length > 0;
