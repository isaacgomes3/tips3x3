 ...
 * Não filtra por admin/estratégia do painel — a extensão decide
 * (enabledMarkets, stake%, lucro, ON/OFF) o que executar.
 * O painel só precisa pedir `autoExt=1` e estar logado; a extensão faz claim.
 */
...
export function publishExtSignalsFromLive(
  ...
  for (const row of rows) {
    ...
    if (qov?.entryReady && !qov.settled && qov.underdogSide) {
    ...
    for (const snap of row.layOverLimitPressure ?? []) {
